...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        #     # 如果是可学习的 logvar，注册为 Parameter，并确保在正确设备上
        #     self.logvar = nn.Parameter(
        #         torch.full((self.num_timesteps,), logvar_init, device=self.device), 
        #         requires_grad=True
        #     )
        # else:
        #     # 如果是固定 logvar，注册为 Buffer，自动同步到模型设备
        #     self.register_buffer('logvar', 
        #         torch.full((self.num_timesteps,), logvar_init)
        #     )
...
class LatentDiffusion(DDPM):
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        # t = t.to(self.logvar.device)  # 将 t 移动到 logvar 的设备
        # logvar_t = self.logvar[t]      # 现在两者在同一设备上
